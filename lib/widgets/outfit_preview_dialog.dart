...
Future<void> showOutfitPreview(BuildContext context, List<ClothingItem> combo) {
  final total = combo.fold(0.0, (s, i) => s + (i.price ?? 0));
  return showDialog(
    context: context,
    builder: (ctx) => Dialog(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            Icon(item.category.iconData, size: 11, color: AppColors.inkSoft),
                            const SizedBox(width: 4),
                            Expanded(
                              child: Text(
                                '${item.subcategory.isNotEmpty ? item.subcategory : item.category.label} · ${fmtPrice(item.price ?? 0)}',
